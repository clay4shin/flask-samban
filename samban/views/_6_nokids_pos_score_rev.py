...
bp = Blueprint('cond6', __name__, url_prefix='/cond6')
...
def index():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if user:
            ...
            r = Reply(participant_id=form.ID.data, create_date=datetime.now(), condition=6)
            db.session.add(r)
            db.session.commit()
            return redirect(url_for('cond6.direct', participant_id=form.ID.data))
...
def direct(participant_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('cond6.result', participant_id=participant_id))
    return render_template('exposure_nokids_pos.html', participant_id=participant_id, form=form)
...
def result(participant_id):
    ...
    return render_template('result_score.html', participant_id=participant_id, score_round=score_round, row=row, condition=6)
...
def revise(participant_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        for row in q:
            ...
            db.session.commit()
            return redirect(url_for('cond6.bye', participant_id=participant_id))
    return render_template('revise_nokids_pos.html', participant_id=participant_id, form=form, pr=pr)
