...
bp = Blueprint('cond1', __name__, url_prefix='/y2s1/cond1')
...
def direct(participant_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('cond1.result', participant_id=participant_id))
...
def result(participant_id):
    ...
    for row in q1:
        ...
        db.session.commit()
    return render_template('y2s1/result_noscore_eng.html', participant_id=participant_id, row=row, condition=1)
...
def revise(participant_id):
    ...
    return render_template('y2s1/revise_gun_neg_high.html', participant_id=participant_id, form=form, pr=pr, condition=1)
