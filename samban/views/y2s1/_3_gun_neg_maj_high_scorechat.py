...
bp = Blueprint('cond3', __name__, url_prefix='/y2s1/cond3')
...
def direct(participant_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('cond3.result', participant_id=participant_id))
...
def result(participant_id):
    ...
    return render_template('y2s1/result_scorechat_eng.html', participant_id=participant_id, score_round=score_round, revised_comment=revised_comment, row=row, condition=3)
...
def revise(participant_id):
    ...
    return render_template('y2s1/revise_gun_neg_high.html', participant_id=participant_id, form=form, rr=rr, condition=3)
