...
bp = Blueprint('cond48', __name__, url_prefix='/y2s1/cond48')
...
def index():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if user:
            ...
            r = Reply(participant_id=form.ID.data, create_date=datetime.now(), condition=48)
            db.session.add(r)
            db.session.commit()
            return redirect(url_for('cond48.direct', participant_id=form.ID.data))
...
def direct(participant_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('cond48.result', participant_id=participant_id))
...
def result(participant_id):
    ...
    return render_template('y2s1/result_scorechat_eng.html', participant_id=participant_id, score_round=score_round, revised_comment=revised_comment, row=row, condition=48)
...
def revise(participant_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        for row in q:
            ...
            db.session.commit()
        return redirect(url_for('cond48.bye', participant_id=participant_id))
    return render_template('y2s1/revise_race_pos_low.html', participant_id=participant_id, form=form, rr=rr, condition=48)
