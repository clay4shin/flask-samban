...
bp = Blueprint('cond40', __name__, url_prefix='/y2s1/cond40')
...
def index():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if user:
            ...
            r = Reply(participant_id=form.ID.data, create_date=datetime.now(), condition=40)
            db.session.add(r)
            db.session.commit()
            return redirect(url_for('cond40.direct', participant_id=form.ID.data))
...
def direct(participant_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('cond40.result', participant_id=participant_id))
...
def result(participant_id):
    ...
    for row in q1:
        ...
        db.session.commit()
    return render_template('y2s1/result_noscore_eng.html', participant_id=participant_id, row=row, condition=40)
...
def revise(participant_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        for row in q:
            ...
            db.session.commit()
        return redirect(url_for('cond40.bye', participant_id=participant_id))
    return render_template('y2s1/revise_race_pos_low.html', participant_id=participant_id, form=form, pr=pr, condition=40)
