...
bp = Blueprint('cond33', __name__, url_prefix='/y2s1/cond33')
...
def index():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if user:
            ...
            r = Reply(participant_id=form.ID.data, create_date=datetime.now(), condition=33)
            db.session.add(r)
            db.session.commit()
            return redirect(url_for('cond33.direct', participant_id=form.ID.data))
...
def direct(participant_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('cond33.result', participant_id=participant_id))
    return render_template('y2s1/exposure_race_pos_high.html', participant_id=participant_id, form=form)
...
def result(participant_id):
    ...
    return render_template('y2s1/result_scorechat_eng.html', participant_id=participant_id, score_round=score_round, revised_comment=revised_comment, row=row, condition=33)
...
def revise(participant_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        for row in q:
            ...
            db.session.commit()
        return redirect(url_for('cond33.bye', participant_id=participant_id))
    return render_template('y2s1/revise_race_pos_high.html', participant_id=participant_id, form=form, rr=rr, condition=33)
