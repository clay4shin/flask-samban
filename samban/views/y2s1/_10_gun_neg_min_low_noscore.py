...
bp = Blueprint('cond10', __name__, url_prefix='/y2s1/cond10')
...
def index():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if user:
            ...
            r = Reply(participant_id=form.ID.data, create_date=datetime.now(), condition=10)
            db.session.add(r)
            db.session.commit()
            return redirect(url_for('cond10.direct', participant_id=form.ID.data))
...
def result(participant_id):
    ...
    for row in q1:
        ...
        db.session.commit()
    return render_template('y2s1/result_noscore_eng.html', participant_id=participant_id, row=row, condition=10)
