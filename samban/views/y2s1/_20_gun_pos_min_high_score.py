...
bp = Blueprint('cond20', __name__, url_prefix='/y2s1/cond20')
...
def index():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if user:
            ...
            r = Reply(participant_id=form.ID.data, create_date=datetime.now(), condition=20)
            db.session.add(r)
            db.session.commit()
            return redirect(url_for('cond20.direct', participant_id=form.ID.data))
...
def direct(participant_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('cond20.result', participant_id=participant_id))
...
def result(participant_id):
    ...
    return render_template('y2s1/result_score_eng.html', participant_id=participant_id, score_round=score_round, row=row, condition=20)
...
def revise(participant_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        for row in q:
            ...
            db.session.commit()
        return redirect(url_for('cond20.bye', participant_id=participant_id))
    return render_template('y2s1/revise_gun_pos_high.html', participant_id=participant_id, form=form, pr=pr, condition=20)
